...
function result = CasimirForceITDLold(t,omegat,PositionX,IT,XiMinOverwrite,XiMaxOverwrite)
...
XiMin=XiMinOverwrite;
XiMax=XiMaxOverwrite;
...
result.y=trapz(k_vec,z,1);
result.x=xi_vec;

plot(xi_vec,result.y);
...
fprintf('For t=%e, x= %e, CasimirF = Vec\n',t,PositionX);
end
...
eta_B_2 = As2Se3old(1i.*xi,2*pi*CONST.c/CONST.lambda0,3);
end
...
epsilon_A = DrudeLorentzold(1i.*xi,2*pi*CONST.c/CONST.lambda0,1);
end

%Dielectric constant for plate B
function epsilon_B = epsilon_B_calc(xi,k,CONST)
epsilon_B = As2Se3old(1i.*xi,2*pi*CONST.c/CONST.lambda0,1);
end

%Permeability for plate A
function mu_A = mu_A_calc(xi,k,CONST)
mu_A = DrudeLorentzold(1i.*xi,2*pi*CONST.c/CONST.lambda0,2);
end

%Permeability for plate B
function mu_B = mu_B_calc(xi,k,CONST)
mu_B = As2Se3old(1i.*xi,2*pi*CONST.c/CONST.lambda0,2);
end
