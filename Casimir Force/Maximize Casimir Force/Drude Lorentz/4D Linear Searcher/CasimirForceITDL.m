...
function result = CasimirForceITDL(PositionX,omega_p_e,omega_t_e,omega_p_m,omega_t_m)
t=0;
...
CONST.omega_p_e=omega_p_e;
CONST.omega_t_e=omega_t_e;
CONST.omega_p_m=omega_p_m;
CONST.omega_t_m=omega_t_m;
...
Kmax=1e9;
myquad = @(fun,a,b,tol,trace,varargin)quadgk(@(x)fun(x,varargin{:}),a,b,'AbsTol',tol);
result=dblquad(@(xi,k) integration_kernel(xi,k,a,0,CONST),0,XiMax,0,Kmax,1e-6,myquad);
...
epsilon_A = 0;%DrudeLorentz(1i.*xi,2*pi*CONST.c/CONST.lambda0,1);
end

%Permeability for plate A
function mu_A = mu_A_calc(xi,k,CONST)
mu_A = 0;%DrudeLorentz(1i.*xi,2*pi*CONST.c/CONST.lambda0,2);
end
...
epsilon_B = DrudeLorentz(1i.*xi,2*pi*CONST.c/CONST.lambda0,1,CONST);
end

%Permeability for plate B
function mu_B = mu_B_calc(xi,k,CONST)
mu_B = DrudeLorentz(1i.*xi,2*pi*CONST.c/CONST.lambda0,2,CONST);
end
