...
function result = CasimirForceIT(t,omegat,PositionX,IT)
...
I = I_calc(t,CONST);
eta_A = sqrt(mu_A.*epsilon_A)+eta_A_2.*I;
end
...
I = I_calc(t,CONST);
eta_B = sqrt(mu_B.*epsilon_B)+eta_B_2.*I;
end
...
test1 = (xi < (CONST.omegat*2*pi*CONST.c/CONST.lambda0)).*1;
test2 = (xi >= (CONST.omegat*2*pi*CONST.c/CONST.lambda0)).*15;
epsilon_A = test1+test2;
end
...
test1 = (xi < (CONST.omegat*2*pi*CONST.c/CONST.lambda0)).*50;
test2 = (xi >= (CONST.omegat*2*pi*CONST.c/CONST.lambda0)).*0.5;
mu_A = test1 + test2;
end
